...
if (file.exists(paste0("Analyses/", run_name, "/samples_to_exclude.txt"))) {
  samples <- readLines(paste0("Analyses/", run_name, "/samples_to_exclude.txt"))
}

ai <- fread(paste0("Analyses/", run_name, "/CN_SNVs_calls.csv"))
if (exists("samples")) {
  ai <- ai[!(sample %in% samples)] 
}

gl <- fread("AnnotationFiles/GeneList_PCF-SELECT_Annotation.tsv")
target_genes <- gl[PCF_SELECT == "yes"]$hgnc_symbol

### Get gene order by pathways
gl_pathways <- gl %>%
  filter(Class != "") %>%
  group_by(Class) %>%
  arrange(sort(hgnc_symbol), .by_group = TRUE) %>%
  as.data.table()

#### Genes to plot
if (all_target_genes) {
  genes <- target_genes
  genes <- gl_pathways$hgnc_symbol
}else{
...
path_anno = rowAnnotation(Pathway = anno_text(gl_pathways$Class, location = 0.5, just = "center"))
...
pdf(paste0("Analyses/", run_name, "/Plots/Oncoprint_pathway_highTC.pdf"), width = 13, height = 14)
onco_byTC_1
dev.off()

pdf(paste0("Analyses/", run_name, "/Plots/Oncoprint_pathway_lowTC.pdf"), width = 13, height = 14)
onco_byTC_2
dev.off()

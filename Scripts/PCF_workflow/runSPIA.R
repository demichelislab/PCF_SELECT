...
mclapply(1:nrow(sif), function(i){
  ...
  #### create SIF for SPIA
  vcfFileList <- normalizePath(paste0(out.spia.sif, gsub(".snps", ".txt", plasma)))
  ...
  ## SPIA table
  outSPIAtable_file = paste0(normalizePath(out.spia.res), "/", gsub(".snps", "", plasma), "_spia.tsv")
  ## SPIA plot file name
  SPIAplot_file = paste0(normalizePath(out.spia.res), "/", gsub(".snps", "", plasma), "_spia_plot.pdf")
  
  ### Prepare config file
  ## create copy of config file
  path_config <- paste0(normalizePath(out.spia.config), "/", gsub(".snps", "", plasma), "_config_file.R")
  ...
  ### Run SPIA
  system(paste("Rscript", normalizePath(path_SPIA), normalizePath(path_config), "2>&1 | tee", paste0(normalizePath(out.spia.res), "/", gsub(".snps", "", plasma), "_spia.log"), sep = " "))
}, mc.cores = cores.SPIA)

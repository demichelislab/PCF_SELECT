...
snvs_splitbychr <- function(id,CHR,snvs_file){
  chrom = CHR[id]
  filename = gsub(basename(snvs_file),pattern = ".pabs",replacement = paste0("_chr",gsub(chrom,pattern = "chr",replacement = ""),".snvs"))
...
for(sname in samplelist){
  dirname <- sname
  message(dirname)
  if(!dirname %in% list.dirs(outdir, full.names=F)){
  	...
  	pileup_file = grep(pileup_list,pattern = paste0("/",dirname,".pileup"),value = T)
  	mclapply(seq(1,length(CHR),1),pileup_splitbychr,CHR=CHR,pileup_file=pileup_file,mc.cores=cores)
  	cat("[ OK ]\n")
  	cat("Writing snvs output by chromosome \t")
  	dir.create(file.path(outdir,dirname,"snvs"))
  	setwd(file.path(outdir,dirname,"snvs"))
  	snvs_list = grep(pacbamlist,pattern = "\\.pabs$",value = T)
  	#snvs_file = grep(snvs_list,pattern = paste(dirname,"pabs",sep="."),value = T)
  	snvs_file = grep(snvs_list,pattern = paste0("/",dirname,".pabs"),value = T)
